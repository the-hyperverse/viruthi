...
const sheetVariants = cva(
  "tw-fixed tw-z-50 tw-gap-4 tw-bg-white tw-p-6 tw-shadow-lg tw-transition tw-ease-in-out data-[state=closed]:tw-duration-300 data-[state=open]:tw-duration-500 data-[state=open]:tw-animate-in data-[state=closed]:tw-animate-out dark:tw-bg-slate-950",
  {
    variants: {
      side: {
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("tw-text-lg tw-font-semibold tw-text-slate-950 dark:tw-text-slate-50", className)}
    {...props}
  />
))
